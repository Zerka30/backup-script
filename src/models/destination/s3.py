...
class S3Bucket:
    ...
    def upload_file(self, file_path, s3_key):
        try:
            ...
            return f"File uploaded to S3 successfully as {s3_key}"
        except Exception as e:
            logging.error(f"Error while uploading {file_path} to S3: {str(e)}")
            return f"Error while uploading {file_path} to S3: {str(e)}"

    def upload_folder(self, folder_path, s3_key):
        pass
    ...
    def delete_folder(self, path):
        files = [
            obj["Key"] for obj in self.list_objects() if obj["Key"].startswith(path)
        ]
        for file in files:
